...
users_db.get '_design/docs', (err, res) ->
  if err
    users_db.save('_design/docs'
      , views:
          by_twitter:
            map: "function(doc) { if (doc.twitterId) emit(doc.twitterId, doc); }"
          by_github:
            map: "function(doc) { if (doc.githubId) emit(doc.githubId, doc); }"
      , (err, res) ->
        console.log "error in creating views for user #{util.inspect(err)}" if err
    )
...
exports.findOrCreate = (source, user_id, user_name, accessToken, accessTokenSecret, promise) ->
  users_db.view "docs/by_#{source}", key: user_id, (err, docs) ->
    if err
      console.log "Error using users/_design/docs/_view/by_#{source} #{util.inspect(err)}"
      promise.fail err
      return
    if docs.length > 0
      user = docs[0].value
